...
bool Data::operator<(const Data &rv_Array)const
{
	// (*this).subject = this->subject
	if ((*this).subject < rv_Array.subject)
		return true;
	return false;
}
...
int main()
{
	...
	cout << "The \"Applying A Data Structure To A Database\" program" << endl << endl;

	SortableStaticArray<Data, 1000> classData;
	...
	while (fin.good())
	{
		...
		if (!found) // when the duplicate is not found
		{
			...
			for (k = 0; k < maxsize; k++)
			{
				if (classData[k].subject == subjectCode)
					break;
			}
