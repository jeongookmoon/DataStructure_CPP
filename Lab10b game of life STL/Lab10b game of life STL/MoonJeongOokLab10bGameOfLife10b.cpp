// Lab 10b, Game Of Life Simulation STL
// Jeong Ook Moon
...
struct Cell
{
	int row; // any +/0/- value
	int col; // any +/0/- value
	bool operator<(const Cell& c) const { return ((row*1000) + (col) < (c.row*1000) + (c.col)); }
};
